...
public class StudentBalanceAgingModule  extends StudentRefundModule {
	...
	private void doOutstandingList() {
		...
		String sqlInvoice = "select new educate.sis.module.SAmount(s.matricNo, SUM(i.amount), i.student, st.period, st.session, st.type) " +
		"from Invoice i join i.student s join s.status st where " +
		" st.session.id = '" + sessionId + "' ";
		if ( !"all".equals(programId)) sqlInvoice += " and s.program.id = '" + programId + "' ";
		if ( !"".equals(intakeId)) sqlInvoice += " and s.intake.id = '" + intakeId + "' ";
		if ( "active".equals(listType)) sqlInvoice += " and st.type.inActive = 0 ";
		sqlInvoice += " group by s.id";
		
	
		String sqlPayment = "select new educate.sis.module.SAmount(s.matricNo, SUM(i.amount), i.student, st.period, st.session, st.type) " +
		"from Payment i join i.student s join s.status st where " +
		" st.session.id = '" + sessionId + "' ";
		if ( !"all".equals(programId)) sqlPayment += " and s.program.id = '" + programId + "' ";
		if ( !"".equals(intakeId)) sqlPayment += " and s.intake.id = '" + intakeId + "' ";
		if ( "active".equals(listType)) sqlPayment += " and st.type.inActive = 0 ";
		sqlPayment += " group by s.id";
		
		List<SAmount> invoiceList = db.list(sqlInvoice);
		System.out.println("Invoice List: " + invoiceList.size());
		List<SAmount> paymentList = db.list(sqlPayment);
		...
		for (SAmount invoiceAmount : invoiceList ) {
			if ( paymentList.contains(invoiceAmount)) {
				int i = paymentList.indexOf(invoiceAmount);
				SAmount paymentAmount = paymentList.get(i);
				double balance = invoiceAmount.getAmount() - paymentAmount.getAmount();
				if ( balance > 0 &&  balance < 0.009 ) balance = 0; //to ignore very small negative amount
				if ( balance > 0 ) {
					
					double agingAmount1 = getPaymentAgingAmountFromList(invoiceAmount.getStudentId(), listAging1);
					double agingAmount2 = getPaymentAgingAmountFromList(invoiceAmount.getStudentId(), listAging2);
					double agingAmount3 = getPaymentAgingAmountFromList(invoiceAmount.getStudentId(), listAging3);
					
					students.add(new RefundItem(invoiceAmount.getStudent(), invoiceAmount.getPeriod(), invoiceAmount.getSession(), invoiceAmount.getStatus(), balance, agingAmount1, agingAmount2, agingAmount3));
					
					//excel
					List cols = new ArrayList();
					rows.add(cols);
					cols.add(invoiceAmount.getStudent() != null ? invoiceAmount.getStudent().getMatricNo() : "NULL");
					cols.add(invoiceAmount.getStudent() != null ? invoiceAmount.getStudent().getBiodata().getIcno() : "NULL");
					cols.add(invoiceAmount.getStudent() != null ? invoiceAmount.getStudent().getBiodata().getName() : "NULL");
					cols.add(invoiceAmount.getPeriod() != null ? invoiceAmount.getPeriod().getName() : "NULL");
					cols.add(invoiceAmount.getStatus() != null ? invoiceAmount.getStatus().getName() : "NULL");
					cols.add(balance);
