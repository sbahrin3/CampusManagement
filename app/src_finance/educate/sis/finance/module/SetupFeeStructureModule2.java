...
public class SetupFeeStructureModule2 extends AjaxModule {
	
	String path = "apps/util/finance/fee_structure2/";
	...
	private void addFee() throws Exception {
		...
		double feeAmt2 = 0.0d;
		try {
			feeAmt2 = Double.parseDouble(amount2);
		} catch ( Exception e ) {}
		
		String amountInt = request.getParameter("amountInt");
		double feeAmtInt = 0.0d;
		try {
			feeAmtInt = Double.parseDouble(amountInt);
		} catch ( Exception e ) {}
		
		String amountInt2 = request.getParameter("amountInt2");
		double feeAmtInt2 = 0.0d;
		try {
			feeAmtInt2 = Double.parseDouble(amountInt2);
		} catch ( Exception e ) {}
		...
		if ( !"".equals(periodId)) {
			...
			if ( "".equals(feeId)) {
				...
				if ( !"".equals(code) && !"".equals(description)) {
					...
					item.setAmount2(feeAmt2);
					item.setAmountInternational(feeAmtInt);
					item.setAmountInternational2(feeAmtInt2);
