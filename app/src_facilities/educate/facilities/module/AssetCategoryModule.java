...
public class AssetCategoryModule extends LebahModule {
	
	private String path = "facilities/asset_category";
	private DbPersistence db = new DbPersistence();

	@Override
	public String start() {
		context.put("categories", db.list("select c from AssetCategory c order by c.sequence"));
	...
	public String updateSequence() throws Exception {
		...
		for ( String id : ids ) {
			i++;
			AssetCategory c = db.find(AssetCategory.class, id);
			db.begin();
			c.setSequence(i);
			db.commit();
		}
	...
	public String addCategory() throws Exception {
		Integer seq = (Integer) db.get("select max(c.sequence) from AssetCategory c");
