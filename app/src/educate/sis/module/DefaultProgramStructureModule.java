...
public class DefaultProgramStructureModule extends AjaxModule {
	
	String path = "apps/util/default_program_structure/";
...
//		context.put("centre", centre);
		...
		SubjectCategory specialize = (SubjectCategory) db.get("select c from SubjectCategory c where c.code = '02'");
		...
		for ( SubjectReg reg : regs ) {
			...
			if ( catId.indexOf("specialisation") == 0 ) {
				System.out.println("this is specialisation category");
				reg.setCategory(specialize);
				
				String specialisationId = catId.substring("specialisation".length() + 1);
				System.out.println("specialisation id = " + specialisationId);
				Specialisation specialisation = (Specialisation) db.find(Specialisation.class, specialisationId);
				
				//reg.setSpecialisation(specialisation);
				
			}
			else {
				SubjectCategory sc = category.get(catId);
				reg.setCategory(sc);
				//reg.setSpecialisation(null);
			}
			
		}
		db.commit();
		
		listSubjectCategories(programId);
	...
	private void listSubjectCategories(String programId) {
		//###.. HARD CODED WARNING HERE!
		//List of Categories
		//02 is the code for specialisation
		List<SubjectCategory> categories = db.list("select c from SubjectCategory c where c.code <> '02' order by c.code");
//		context.put("categories", categories);
		
		//list of specialisations
		List<Specialisation> specialisations = db.list("select s from Specialisation s where s.program.id = '" + programId + "'");
		
		
		Vector<Hashtable> cats = new Vector<Hashtable>();
		for ( SubjectCategory category : categories ) {
			Hashtable cat = new Hashtable();
			cat.put("id", category.getId());
			cat.put("code", category.getCode());
			cat.put("name", category.getName());
			cats.add(cat);
		}
		
		if ( specialisations.size() > 0 ) {
			Hashtable catsep = new Hashtable();
			catsep.put("id", "specialisation");
			catsep.put("code", "");
			catsep.put("name", "SELECT SPECIALISATION");
			cats.add(catsep);
			
			for ( Specialisation specialisation : specialisations ) {
				Hashtable cat = new Hashtable();
				cat.put("id", specialisation.getId());
				cat.put("code", specialisation.getCode());
				cat.put("name", specialisation.getName());		
				cats.add(cat);
			}
		}
	...
	private void getStructureByCentre() throws Exception {
		...
		String sql = "";
		sql = "select p from ProgramStructure p where p.program.id = '" + programId + "' " +
				"and p.learningCenter.id is null " +
