...
public class StudentListModule2 extends AjaxModule {
	
	String path = "apps/util/student_list2/";
	...
	private void listStudents() {
		...
		if ( program != null && intake != null && session != null ) {
			...
			if ( status != null ) {
				h.put("status", status);
				studentStatus = db.list("select s from StudentStatus s where s.student.program = :program and s.student.intake = :intake and s.session = :session and s.type = :status order by s.student.biodata.name", h);
			} else
				studentStatus = db.list("select s from StudentStatus s where s.student.program = :program and s.student.intake = :intake and s.session = :session order by s.student.biodata.name", h);
		}
			...
			if ( status != null ) {
				h.put("status", status);
				studentStatus = db.list("select s from StudentStatus s where s.student.intake = :intake and s.session = :session and s.type = :status order by s.student.biodata.name", h);
			}else
				studentStatus = db.list("select s from StudentStatus s where s.student.intake = :intake and s.session = :session order by s.student.biodata.name", h);
			...
			if ( status != null ) {
				h.put("status", status);
				studentStatus = db.list("select s from StudentStatus s where s.session = :session and s.type = :status order by s.student.biodata.name", h);
			} else
				studentStatus = db.list("select s from StudentStatus s where s.session = :session order by s.student.biodata.name", h);
		}
		else if ( program != null && intake == null && session != null ) {
			h.put("program", program);
			h.put("session", session);
			...
			if ( status != null ) {
				h.put("status", status);
				studentStatus = db.list("select s from StudentStatus s where s.student.program = :program and s.session = :session and s.type = :status order by s.student.biodata.name", h);
			} else
				studentStatus = db.list("select s from StudentStatus s where s.student.program = :program and s.session = :session order by s.student.biodata.name", h);
		}
